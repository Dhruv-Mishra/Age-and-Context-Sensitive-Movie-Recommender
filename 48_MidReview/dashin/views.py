...
def movie_recom(request):
    if not request.user.is_authenticated:
        ...
        usr = request.user
        profile = models.Profile.objects.filter(user=usr).first()
        search = models.Search(keyword = query)
        


        movie_recoms_info = start(query)
        #print(movie_recoms_info)
        movie_recoms = movie_recoms_info['Movie Title'].tolist()
        # for i in movie_recoms_info['Movie Title']:
        #     movie_recoms.append(i)
        ...
        for i in movie_recoms:
            m = models.Movie.objects.all().filter(movie_name=i).first()
            movie_recoms_objects.append(m)
        ...
        for movie in movies_list:
            cur_mov = models.Movie.objects.all().filter.first()
            if movie[1] == 'true':
